...
- Site-mirror fail-static / health-gated auto-rollback lives in `worker/src/lib/fail_static.mjs`
  + `worker/src/mirror.mjs` (LKG pin in `ALERT_STATE`, `X-CityScroll-Serve` mode header). Operator
  runbook: `docs/fail-static-serving.md`. After changing either copy run
