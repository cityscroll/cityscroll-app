...
- Static hosting is dual during migration: GitHub Pages remains the public production origin
  ...
  in ``; do not change DNS from CI. The API Worker stays on
  Cloudflare either way. Visitor-facing source lives under `site/`; `.github/actions/build-site/action.yml`
  preserves legacy-root builds for rollback.
...
- Hearing location extraction is deliberately dual-implemented in `site/hearing_location.js` and
