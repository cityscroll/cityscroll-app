workspace {
    ...
    model {
        ...
        performance_registry -> browser_site "Projects the classification manifest and closed collector contract for the bounded production RUM pilot [architecture/performance-observability.v1.json; site/data/performance-classification-manifest.v1.json; site/rum_collector.mjs]" "Repository artifact"
        ...
        browser_site -> worker_api "Submits bounded first-party RUM observations only to /performance-events from canonical production hosts when both independent switches are on [architecture/performance-observability.v1.json; site/rum_collector.mjs; site/rum_delivery.mjs; site/rum_production.mjs]" "HTTPS"
        private_desk -> worker_api "Reads the private /admin/performance model through a server-to-server authenticated proxy; no Analytics Engine credentials cross into Desk [architecture/performance-observability.v1.json; data/rum-09-desk-contract-fixtures/desk-consumer-contract.v1.json]" "HTTPS"
