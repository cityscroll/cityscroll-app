...
import { calendarFeedUrlForScope, scopeFromLensState } from "../site/scope_v0.mjs";
import {
...
test("A1 subscription emits exact observe scope and feed replay keeps collection identities", async () => {
  await withPinnedClock(PINNED_OBSERVED_AT, () => {
    ...
    assert.equal(
      calendarFeedUrlForScope(scopeFromLensState("meetings", { body: "bsa_calendar", place_role: "venue" })),
      null,
      "A1 standing feed declines place role rather than silently dropping it",
    );
