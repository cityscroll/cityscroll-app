...
test("records a content-addressed WebP with CI review references and retention", async () => {
  await withTempDir("evidence-store", async (dir) => {
...
test("rejects a local filesystem review URL", async () => {
  await withTempDir("evidence-store", async (dir) => {
    const capture = join(dir, "capture.webp");
    writeFileSync(capture, Buffer.from("RIFF0000WEBP fixture bytes"));
    const result = run([
...
test("release evidence has indefinite retention and a backstage URL locally", async () => {
  await withTempDir("evidence-store", async (dir) => {
    const capture = join(dir, "capture.webp");
