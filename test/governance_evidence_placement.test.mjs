...
test("retained public policy, contracts, and merge-throughput evidence stay accepted", () => {
  ...
  assert.ok(existsSync("docs/repository-governance/evidence-placement-shard.v1.schema.json"));
...
test("private schemes and raw inventory rows stay rejected from retained public content", () => {
  const directory = mkdtempSync(join(tmpdir(), "governance-guard-"));
  try {
...
test("reintroducing the whole-repository aggregate fails the check", () => {
  ...
  try {
    mkdirSync(join(root, "docs", "repository-governance"), { recursive: true });
...
test("the placement check passes on the current tip and under a merge-group ref", () => {
  const output = execFileSync("node", ["tools/governance_evidence_placement.mjs", "--check"], { encoding: "utf8" });
  ...
  const mergeGroupOutput = execFileSync("node", ["tools/governance_evidence_placement.mjs", "--check"], {
    encoding: "utf8",
...
test("an ambient GIT_DIR does not steer the placement check", () => {
  ...
  try {
    const output = execFileSync("node", ["tools/governance_evidence_placement.mjs", "--check"], {
      encoding: "utf8",
...
test("the derived receipt cannot be written back into the tracked tree", () => {
  const result = spawnSync("node", ["tools/governance_evidence_placement.mjs", "--write", "--output-dir", "docs/repository-governance"], { encoding: "utf8" });
