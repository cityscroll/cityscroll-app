...
test("refresh execution completes acquisitions before rebuilding affected artifacts", () => {
  ...
  assert.deepEqual(calls.map((call) => call[1]), [
    ...
    "/fixture/tools/materialize.mjs",
  ]);
  assert.deepEqual(receipt.commands.map((row) => row.kind), [
    "acquisition", "acquisition", "owning-builder", "dependent-materializer",
  ]);
});
...
// Every committed derived artifact whose CI or test suite asserts it is reproducible from
// current first-class data must be regenerated by the scheduled refresh, or that assertion
// will fail whenever the refresh advances the underlying data without regenerating it.
// Add new entries here (generator, plus the source data it derives from) whenever a new
// committed-and-checked derived artifact is introduced.
const COMMITTED_DERIVED_ARTIFACTS_WITH_FRESHNESS_ASSERTIONS = [
  {
    artifact: "worker/src/data/keyword_search_index_shards/",
    generator: "tools/build_keyword_search_index.mjs",
  },
  {
    artifact: "site/data/land_project_map_points_receipt.json",
...
test("the declared materializer graph regenerates every committed derived artifact with a freshness assertion", () => {
  const registry = canonical();
  const declaredCommands = new Set(registry.first_class_artifacts.flatMap((row) => [
    ...row.dependent_materializers,
    row.builder_command.join(" "),
    row.acquisition_command.join(" "),
  ]));
  for (const { artifact: derivedArtifact, generator } of COMMITTED_DERIVED_ARTIFACTS_WITH_FRESHNESS_ASSERTIONS) {
    assert.ok(
      declaredCommands.has(generator) || [...declaredCommands].some((command) => command.startsWith(`node ${generator}`)),
      `${derivedArtifact} has a freshness assertion but ${generator} is not run by the first-class refresh (add it to a dependent_materializers list in site/data/source_contracts.json)`,
    );
  }
});
