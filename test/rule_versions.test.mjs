...
test("committed Tier-2 receipt keeps characterization and bridge metrics separate", () => {
  ...
  assert.equal(materialization.checks.characterization.agencies.length, 8);
  assert.equal(materialization.checks.rule_documents.bounded_document_denominator, 10);
  assert.equal(materialization.checks.rule_documents.proposed_documents, 6);
  assert.equal(materialization.checks.rule_documents.adopted_documents, 4);
  assert.equal(materialization.checks.rule_documents.cases_with_both_versions, 1);
  assert.equal(materialization.checks.rule_documents.acquisition_failures, 5);
  assert.equal(materialization.checks.rule_documents.source_not_published, 1);
  assert.equal(materialization.checks.rule_documents.fetch_not_attempted, 1);
  assert.equal(materialization.checks.rule_documents.extracted_empty, 1);
  assert.equal(materialization.checks.rule_documents.scanned_or_non_text, 1);
  assert.equal(materialization.checks.version_pairing.proposed_adopted_pairs, 1);
  assert.equal(materialization.checks.version_pairing.source_id_pairing_evidence[0], "shared_source_pairing_key");
  assert.equal(materialization.checks.version_pairing.pair_rate.denominator, 4);
  assert.ok(Object.hasOwn(materialization.checks.legal_citations, "ambiguous_references"));
  assert.ok(materialization.checks.legal_citations.ambiguous_references > 0);
  assert.ok(materialization.checks.legal_citations.resolvable_targets > 0);
  assert.ok(materialization.checks.legal_citations.canonical_resolutions_by_corpus.rcny > 0);
  assert.ok(materialization.checks.legal_citations.canonical_resolutions_by_corpus["nyc-charter"] > 0);
  assert.ok(materialization.checks.legal_citations.canonical_resolutions_by_corpus["nyc-admin-code"] > 0);
  assert.equal(materialization.checks.version_diff.usable_version_pairs, 1);
  assert.equal(materialization.checks.version_diff.text_extraction.available_versions, 5);
  ...
  assert.ok(materialization.projections[0].versions.every((version) => version.source_document.publisher));
  assert.ok(materialization.projections[0].versions.every((version) => version.source_document.observed_at));
  assert.equal(new Set(materialization.projections[0].versions.map((version) => version.id)).size, materialization.projections[0].versions.length);
  const external = materialization.projections[0].versions.find((version) => version.source_id === "city_record:singleton-external");
  assert.equal(external.legal_effects.length, 0);
});
