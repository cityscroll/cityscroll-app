...
// bounded reads, and a clean invalid-tool failure. It is NOT part of the fast
// `test/*.test.mjs` sweep (see .github/workflows/ci.yml) — it runs from
// .github/workflows/deploy-worker.yml's post-deploy smoke job instead, which
// is also where the card's negative rule ("do not exercise watch creation ...")
// and boundary rule ("no fetch override, no handleMcp() import, no fixture
// environment, no local Worker dispatch") are asserted against this file's and
// the canary's own source text, not just against one run's behavior.
//
...
import { EVIDENCE_CLASSES, EXECUTION_ENVIRONMENTS, scanSourceForHandleMcpImport } from "../capabilities/evidence_classification.mjs";
import { runLiveMcpCanary } from "../tools/verify_live_remote_mcp_canary.mjs";
...
test("A9: local protocol tests remain fast and independent of the network", () => {
  ...
  assert.match(stepBlock, /test\/\*\.test\.mjs/, "expected the existing fast test/*.test.mjs sweep in ci.yml");
  assert.match(stepBlock, /live_mcp_canary/, "the fast sweep must explicitly exclude the live canary file");
