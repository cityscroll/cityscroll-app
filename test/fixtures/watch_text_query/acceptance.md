...
| A5 legacy identity and dispatch | `worker/test/subscriptions.test.mjs`; compiler/feed refusals | Absent `text_query` keeps the legacy canonical string. Equivalent v1 expressions share one identity; an intentional change differs. Modern feeds still refuse a v1 watch. Money compilers evaluate through the procurement adapter rather than running unfiltered; other lenses still refuse. |
