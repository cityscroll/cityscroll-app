...
| A5 legacy identity and dispatch | `worker/test/subscriptions.test.mjs`; compiler/feed refusals | Absent `text_query` keeps the legacy canonical string. Equivalent v1 expressions share one identity; an intentional change differs. Compilers and modern feeds refuse a v1 watch rather than run it unfiltered. |
