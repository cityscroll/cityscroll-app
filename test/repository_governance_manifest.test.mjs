...
import { buildManifest, scanUnclassifiedFixture, validateManifest } from "../tools/repository_governance_manifest.mjs";

test("landed manifest preserves every required classification candidate", () => {
  const manifest = JSON.parse(readFileSync(new URL("../docs/repository-governance/classification.v1.json", import.meta.url), "utf8"));
...
test("unclassified planning, owner decisions, and private evidence fail with named findings", () => {
  const path = "test/fixtures/repository_governance/negative/unclassified.md";
...
test("accepted ADR and implementation evidence fixtures pass", () => {
  const paths = [
    "test/fixtures/repository_governance/positive/accepted-adr.md",
    "test/fixtures/repository_governance/positive/code-evidence.json",
    "test/fixtures/repository_governance/positive/test-example.mjs",
    "test/fixtures/repository_governance/positive/fixture-example.json",
    "test/fixtures/repository_governance/positive/mt7-shard.json",
  ];
