#!/usr/bin/env python3
"""PHC-02 evidence: render the meeting detail page's new "What this means" consequence
block (site/meeting_document.mjs's renderMeetingDocument(), via
site/meeting_purpose_authority.mjs) for five synthetic City Record records, one per state
named in the card — sourced purpose/authority with a matched next official action, an
unsourced descriptive meeting, listen-only vs. testimony participation, and a held meeting
with no published outcome. Runs axe-core against each rendered document at both review
widths.
...
(spec.md).
  ...
  node tools/capture_phc02_purpose_and_authority.mjs   # (invoked by this script)
  python3 tools/capture_phc02_purpose_and_authority.py
"""
...
def start_local_site_server() -> tuple[subprocess.Popen, str]:
    ready_file = Path(tempfile.mktemp(prefix="crol-phc02-site-"))
...
def main() -> None:
    ...
    generated = subprocess.run(
        [shutil.which("node") or "node", str(ROOT / "tools/capture_phc02_purpose_and_authority.mjs")],
        ...
        with sync_playwright() as p:
            ...
            for viewport_name, width, height in VIEWPORTS:
                ...
                for case in cases:
                    ...
                    consequence = page.locator('[data-meeting-consequence="1"]')
                    has_consequence = consequence.count() > 0
                    consequence_html = consequence.inner_html() if has_consequence else ""
                    ...
                    print(f"{viewport_name:8s} {case['id']:45s} consequence={has_consequence!s:5} "
