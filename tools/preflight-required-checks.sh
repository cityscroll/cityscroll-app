...
family_static_standards() {
  ...
  run_and_fail node tools/build_land_project_catalog.mjs --check
  run_and_fail node tools/build_land_place_membership.mjs --check
  run_and_fail node tools/land_place_refresh.mjs --check
  ...
  run_and_fail node tools/build_community_board_district_projects.mjs --check
  run_and_fail node tools/build_community_board_land_positions.mjs --check
  ...
  run_and_fail node tools/build_agency_entity_publication.mjs --check
...
if [[ "$RUN_FULL" == "1" ]]; then
  ...
  # Agency constellation HTML is gitignored; generate it before the public
  # artifact copy so axe, demo-links, and hash-route checks hit the same
  # static documents as production.
  ...
  run_and_fail python3 test/functional/42_search_recent_history.py
  ...
  run_and_fail python3 test/functional/42_land_map_marker_selection.py
