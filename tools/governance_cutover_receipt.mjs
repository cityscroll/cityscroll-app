...
/**
 * Repository governance cutover proof.
 *
 ...
 * docs/repository-governance/cutover.d/, one file per semantic key, so two
...
export const SHARD_SCHEMA = "cityscroll.repository-governance.cutover-shard.v1";
export const CARD = "cityscroll-engineering/cutover-without-product-or-evidence-loss";
export const SHARD_DIRECTORY_RELATIVE = "docs/repository-governance/cutover.d";
export const FORBIDDEN_AGGREGATE_RELATIVE = "docs/repository-governance/cutover.v1.json";
export const DERIVED_AGGREGATE_RELATIVE = ".artifacts/repository-governance/cutover.v1.json";

const CLASSIFICATION_RELATIVE = "docs/repository-governance/classification.v1.json";
const OWNER_MAPPING_RELATIVE = "docs/repository-governance/semantic-owner-mapping.v1.json";
const GUARD_MANIFEST_RELATIVE = "docs/repository-governance/inverse-guard.v1.json";
const ROUTER_POLICY_RELATIVE = "docs/repository-governance/agents-router-policy.v1.json";
const PLACEMENT_TOOL_RELATIVE = "tools/governance_evidence_placement.mjs";
...
export function migrationKeyForEntry(entry) {
  if (entry.path === "AGENTS.md") return "root-router";
  if (TEMPORAL_CLASSES.has(entry.content_class)) return "shadow-planning";
  if (PRIVATE_CLASSES.has(entry.content_class)) return "private-evidence";
...
/**
 * Resolve the semantic-owner mapping's indirection back to a concrete owner.
 *
...
function main(argv = process.argv.slice(2)) {
  ...
  if (findings.length) {
    for (const finding of findings) process.stderr.write(`repository governance cutover: ${finding}\n`);
    process.exitCode = 1;
    return;
  }
  const migrated = receipt.migrations.reduce((sum, row) => sum + row.manifest_entry_count, 0);
  process.stdout.write(
    `repository governance cutover verified: ${migrated} classified entries across ${receipt.migrations.length} bounded migrations, `
