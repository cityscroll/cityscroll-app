...
const PATTERN_ROWS = [
  ...
  ["monitor pack document", "/following/#packs", "/following/packs/{pack_id}/", "The pack owns its member watches and keeps each watch available through the unified subscribe flow.", "The Following pack card links to the canonical pack document.", "/following/packs/restaurants/"],
  ["district digest document", "/following/?lens=district&council={id}", "/districts/council/{id}/digest/", "The digest owns the typed council-district pivot and weekly watch action.", "The prior Following watch remains a supported view.", "/districts/council/33/digest/"],
  ...
  ["People directory view", "/#people?{filters}", "/browse/people/?{filters}", "Forward only the People directory allowlist; mixed Staffing or Exam keys are disclosed as unsupported.", "Legacy root location.replace() translates the input once, then the People document owns runtime state.", "/#people?q=Rodriguez"],
  ["Staffing appointments view", "/#staffing?{filters}", "/browse/staffing/?{filters}", "Forward only the Staffing appointment allowlist; mixed People or Exam keys are disclosed as unsupported.", "Legacy root location.replace() translates the input once, then the Staffing document owns runtime state.", "/#staffing?role=Engineer"],
  ["Legacy exam-guide intent", "/#people?view=guide or /#staffing?view=guide or /#exams", "/browse/exams/?{filters}", "Consume view=guide as intent and forward only the Exam filter allowlist.", "Legacy root location.replace() translates the input once, then the Exams document owns runtime state.", "/#staffing?lang=es&view=guide"],
  ...
  ["public Stats document", "/stats.html", "/stats.html", "URL unchanged; content is intentionally limited to served-product coverage facts.", "Explicitly excluded from route rewrites and the Pages edge route.", "/stats.html"],
  ["public Stats API", "https://api.cityscroll.org/stats", "https://api.cityscroll.org/stats", "URL unchanged; public-stats.v4 publishes served-product coverage plus verified search counts for named periods, and keeps search queries, readers and delivery fields on authenticated /admin/stats.", "Explicitly excluded from the site route migration.", "https://api.cityscroll.org/stats"],
