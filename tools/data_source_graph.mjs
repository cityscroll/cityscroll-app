...
</style><noscript><style>#graphView,#glanceView{display:none!important}#tableView[hidden],#detailLayer[hidden]{display:block!important}</style></noscript></head><body><main class="shell">
...
function selectedSourceUrl(id){const url=new URL(window.location.href);url.searchParams.set("source",id);url.hash="source-"+id;return url}
function revealSelectedSourceLayer(){const glance=document.getElementById("glanceView"),layer=document.getElementById("detailLayer"),glanceToggle=document.getElementById("glanceToggle"),detailToggle=document.getElementById("detailToggle");if(glance&&layer){glance.hidden=true;layer.hidden=false;glanceToggle?.setAttribute("aria-pressed","false");detailToggle?.setAttribute("aria-pressed","true")}}
function selectSource(id,{updateUrl=true,replaceUrl=false}={}){
  selected=id;
  revealSelectedSourceLayer();
  const s=graph.sources.find(x=>x.id===id);
  if(!s){details.innerHTML='<div class="empty-detail" role="status"><div class="eyebrow">Source detail</div><h2>Source not found</h2><p>No source matches <strong>'+escapeHtml(id)+'</strong>. The remaining source register is still available.</p></div>';render();return false}
...
search.addEventListener("input",render);statusFilter.addEventListener("change",render);document.querySelectorAll(".table-source").forEach(link=>link.addEventListener("click",event=>{event.preventDefault();selectSource(link.dataset.source);setView("graph")}));
function setView(view){const views={graph:"graphView",table:"tableView",repair:"repairView"},toggles={graph:"graphToggle",table:"tableToggle",repair:"repairToggle"};for(const [name,id] of Object.entries(views)){const node=document.getElementById(id);if(node)node.hidden=name!==view;const toggle=document.getElementById(toggles[name]);if(toggle)toggle.setAttribute("aria-pressed",String(name===view))}}
function restoreSelectionFromUrl(){const id=new URLSearchParams(window.location.search).get("source");if(id)selectSource(id,{updateUrl:false});else{selected=null;details.innerHTML=emptyDetails;render()}}
document.getElementById("graphToggle").onclick=()=>setView("graph");document.getElementById("tableToggle").onclick=()=>setView("table");document.getElementById("repairToggle").onclick=()=>setView("repair");window.addEventListener("popstate",restoreSelectionFromUrl);restoreSelectionFromUrl();
</script></body></html>\n`;
}
