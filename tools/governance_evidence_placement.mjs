 ...
 * docs/repository-governance/evidence-placement.d/. Each shard owns exactly one
...
export const SHARD_SCHEMA = "cityscroll.repository-governance.evidence-placement-shard.v1";
export const SHARD_DIRECTORY_RELATIVE = "docs/repository-governance/evidence-placement.d";
export const FORBIDDEN_AGGREGATE_RELATIVE = "docs/repository-governance/evidence-placement.v1.json";
export const DERIVED_AGGREGATE_RELATIVE = ".artifacts/repository-governance/evidence-placement.v1.json";
...
const RETAINED_PROOF_PATHS = [
  ...
  "docs/repository-governance/evidence-placement.d/README.md",
  "docs/repository-governance/evidence-placement-shard.v1.schema.json",
...
/**
 * The inspected commit is a fixed point in the past, so it spells a path the way that
 * commit spelled it rather than the way the current tree does. Resolve the record out
 * of that commit's own tree by the file name it has always carried, so the derivation
 * stays correct across a rename of the directory without pinning either spelling here.
 */
function pathAtBase(fileName) {
  const matches = git(["ls-tree", "-r", "--name-only", BASE], { encoding: "utf8" })
    .split("\n")
    .filter((path) => path.endsWith(`/${fileName}`));
  if (matches.length !== 1) {
    throw new Error(`expected exactly one ${fileName} at the inspected commit; found ${matches.length}`);
  }
  return matches[0];
}
...
export function derivePlacementFacts() {
  const classification = JSON.parse(fromBase(pathAtBase("classification.v1.json")));
...
export function verifyTip(receipt, { root = ROOT } = {}) {
  ...
  const placementPaths = [
    ...
    "tools/governance_evidence_placement.mjs"
  ];
