...
- **Community boards as dual-role civic bodies.** Board identity is publisher-keyed by `body_id` and shared by place and organization projections. Geographic `covers` remains separately gated; organization relation families are declared without unsourced edges, and the aggregate Community Boards agency remains an index. Rationale: `rationale required`.
...
- **Architecture facts link here.** LA4's machine-owned evidence artifact is [`architecture/generated/facts.json`](architecture/generated/facts.json), generated by [`tools/build_architecture_facts.mjs`](tools/build_architecture_facts.mjs) and checked against the C4 model by [`tools/reconcile_architecture.mjs`](tools/reconcile_architecture.mjs). The artifact records observed configuration and ontology counts; it does not supply design rationale. Rationale: `rationale required`.
