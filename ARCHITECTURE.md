...
- **Future architecture facts link here.** LA4 is expected to generate verified topology and configuration facts. No generated artifact is present in this change, so its path and schema remain unset rather than guessed. Rationale: `rationale required`.
