...
function comparisonValue(kind, value) {
  if (kind === "amount") return comparisonAmount(value);
  if (kind === "date") return comparisonDate(value);
  return clean(value) || null;
}

function sourceAssertion(observation, definition) {
  const snapshot = objectSnapshot(observation?.raw_snapshot);
  ...
  const compared = comparisonValue(definition.kind, value);
...
export function buildAssertionEvidence(left = {}, right = {}) {
  ...
  for (const definition of FACTS) {
    const leftAssertion = sourceAssertion(left, definition);
    const rightAssertion = sourceAssertion(right, definition);
