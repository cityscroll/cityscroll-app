...
| `/admin/subs` `/admin/feedback` | GET | Operator reads. Subscriptions include status, source, creation time, and recovery provenance when present. Feedback projects stored report target, evidence, provenance, canonical URL, and target identity with explicit missing/malformed context; reporter email, IP, user-agent, and adjudication notes are omitted | `ADMIN_KEY` → 404 if unset |
