...
| `/admin/reliability/digest` | GET | Daily digest dead-man check for the shadow READY receipt, terminal delivery receipt, queue reconciliation, and dead-letter count; returns 503 and sends an operations alert when unhealthy | `ADMIN_KEY` → 404 if unset; `ALERT_STATE` |
