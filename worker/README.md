...
| `/stats` | GET | **Served-product coverage facts** (`public-stats.v3`): the materialised coverage snapshot — per served record set, its record unit, counting rule, count and own evidence date — plus language coverage. Built from `site/data/served_coverage_snapshot.json`, so the route never fetches a publisher; product usage, subscriptions, sends, and daily series are intentionally absent; edge-cached 15 min. | none |
...
| `/admin/reliability/mail` | GET/POST | Mail-leg health. POST `{action:"canary"}` sends a Resend probe to `subscribe@crol-list.org` and one operations-mailbox canary; GET returns 503 when a canary is unmatched or outbound send failed. The Gmail forward leg is dashboard-gated and stays `unprobed`. This route does not email a dead mail rail | `ADMIN_KEY` → 404 if unset; `ALERT_STATE` |
