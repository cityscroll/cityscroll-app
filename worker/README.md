...
| `/admin/reliability/digest` | GET | Daily digest dead-man check for the shadow READY receipt, terminal delivery receipt, queue reconciliation, dead-letter count, and consecutive-delivery watermark staleness; returns 503 and sends an operations alert when unhealthy | `ADMIN_KEY` → 404 if unset; `ALERT_STATE` |
| `/admin/reliability/scheduler` | POST/GET | Records an external scheduler heartbeat or checks heartbeat age and pending outbox items; returns 503 and sends an operations alert when unhealthy | `ADMIN_KEY` → 404 if unset; `ALERT_STATE` |
