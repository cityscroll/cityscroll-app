...
| `/entity-dossier?id=` | GET | Public, read-only entity dossier with linked source records, attributed assertions, explicit disagreement/missingness, and an allowlisted sensitivity boundary; HTML by default, JSON with `Accept: application/json` or `?format=json`; edge-cached 5 minutes | none; `DB` |
