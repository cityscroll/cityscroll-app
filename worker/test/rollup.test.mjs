...
test("isWatchActive: developer and e2e plus-tag addresses never receive real digests", () => {
  ...
  assert.equal(isWatchActive(sub("reader-real@example.com", "sub:real")), true);
  const jobs = buildDigestJobs([
    sub("jamesca2ro+scope-watch-e2e-20260806@gmail.com", "sub:e2e"),
    sub("reader-real@example.com", "sub:real"),
  ]);
  assert.deepEqual(jobs.map((job) => job.email), ["reader-real@example.com"]);
});
