...
test("a missing-topic signup without the disclosed homepage source requires a selected watch and creates no record", async () => {
  const environment = env();
  const response = await handleSubscribe(new Request("https://api.cityscroll.org/subscribe", {
    method: "POST",
    headers: { "content-type": "application/json", origin: "https://cityscroll.org" },
    body: JSON.stringify({ email: EMAIL, no_topic: true, source: "some-other-caller" }),
  }), environment);
