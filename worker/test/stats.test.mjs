...
test("GET /stats publishes all-time totals + category breakdown alongside the unchanged 7-day fields", async () => {
  ...
  assert.deepEqual(Object.keys(body.digests).sort(), ["by_category", "catch_up_last_run", "catch_up_sent_all_time", "catch_up_sent_today", "lagging_subs", "last_run", "sent_all_time", "sent_last7d", "sent_today"]);
  assert.equal(body.digests.sent_today, 0);
  assert.equal(body.digests.sent_last7d, 0);
  assert.equal(body.digests.last_run, null);
  assert.equal(body.digests.catch_up_sent_today, 0);
  assert.equal(body.digests.catch_up_sent_all_time, 0);
  assert.equal(body.digests.catch_up_last_run, null);
  assert.equal(body.digests.lagging_subs, 0);
