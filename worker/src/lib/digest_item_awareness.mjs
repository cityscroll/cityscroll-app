// Digest email time-awareness + action-awareness (render/content only).
//
// Mirrors the site action rails (site/action_registry.js) and deadline chips
// (open / closing-soon / closed from EVENT time) so email items stop punting
// with a generic "see the notice" when ingested fields already support a
// concrete next step.
//
// Pure: no I/O, no send-path side effects. Does NOT touch seen: keys or
// reconcileTemporalCandidates — delivery identity stays in alert_temporal.mjs.
...
import {
  ...
  zoningHandoff,
  franchiseHandoff,
} from "./action_registry.mjs";
import { shortDate } from "./digest.mjs";
...
function deadlineStatusCopy(deadline, lang) {
  ...
  if (state === "rolling") return emailT(lang, "digest_deadline_rolling");
  if (state === "closed") {
    return date
      ? emailT(lang, "digest_deadline_closed_on", { date })
      : emailT(lang, "digest_deadline_closed");
  }
  if (state === "closing-soon") {
    if (n === 0) return emailT(lang, "digest_deadline_closes_today");
    if (n === 1) return emailT(lang, "digest_deadline_closes_tomorrow");
    if (date && n != null) {
      return emailT(lang, "digest_deadline_closing_soon", { date, n });
    }
    return emailT(lang, "digest_deadline_closing_soon_bare");
  }
  // open
  if (date && n != null) return emailT(lang, "digest_deadline_open", { date, n });
  if (date) return emailT(lang, "digest_deadline_open_date", { date });
  return emailT(lang, "digest_deadline_open_bare");
}
...
export function itemAwarenessHtml(row, esc, lang = "en", opts = {}) {
  ...
  if (temporal?.kind === "rules-comment-open" && temporal.event_at) {
    status = emailT(lang, "rules_comment_open", { date: shortDate(temporal.event_at) });
  } else {
  ...
  if (temporal?.kind === "rules-comment-open" && temporal.url) {
    ...
    const label = text(a.label) || emailT(lang, "digest_next_action_default");
    if (a.destination && a.delivery === "official_handoff") {
      parts.push(
        `<div style="font-size:13px;margin:2px 0"><b>${esc(emailT(lang, "digest_next_action_label"))}</b> `
