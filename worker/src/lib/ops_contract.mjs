...
export const DIGEST_SHADOW = Object.freeze({
  ...
  observation_codes: ["quiet_watermark", "expected_catch_up_explosion"],
  monitoring: {
    ...
    expected_catch_up_explosion_policy: "An aggregate spike above the trailing baseline is an informational observation when owed_drained exceeds content_deduped by the explosion ratio or a preview uses the catch-up subject form. The receipt carries the full selection funnel; fresh-dominant spikes remain aggregate_count_explosion redlines.",
...
export const STATS_METRICS = Object.freeze([
  ...
  {
    path: "mcp_usage.*",
    source: "USAGE_ANALYTICS MCP taxonomy + authenticated /admin/stats fold",
    exclude_developer_traffic: true,
    description: "Machine-protocol observations on POST /mcp; production observation_class only in product totals; developer/canary/probe kept as separate classes.",
  },
  {
...
export const ADMIN_ROUTES = Object.freeze([
  ...
  },
  {
    ...
    methods: ["POST"],
    auth: "ADMIN_KEY",
    description: "Private relay for operational alarms into Desk and the bounded repair queue. Routine findings and repair judgments do not send email; only a validated production-emergency incident is email eligible, with D1 ownership and provider idempotency keyed by its canonical signature. Scheduler and served-artifact guards must supply workflow, source revision, a validated run link and a validated raw-receipt link, otherwise the relay refuses with 400.",
  },
  {
    path: "/admin/reliability/digest",
    methods: ["GET"],
    auth: "ADMIN_KEY",
    description: "Digest dead-man check including consecutive-delivery watermark staleness and mail-leg receipts; returns 503 and GitHub-red when unhealthy. Findings are recorded in Desk and offered to the bounded repair queue without routine operations email.",
  },
  {
    path: "/admin/reliability/scheduler",
    methods: ["POST", "GET"],
    auth: "ADMIN_KEY",
    description: "POST records one external scheduler heartbeat and accepts it only with concrete workflow, run identity, source revision and cycle result; an unattributable heartbeat is rejected with 400 and never stored. POST with cycle=desk-publication records the Desk evidence-publication heartbeat on the same route and store without replacing the scheduler heartbeat. GET reports scheduler liveness, Desk publication liveness, and mail-leg receipts as separate legs and returns 503 when unhealthy; findings carry the observing workflow, run link, source revision and this route as the raw receipt, remain Desk-only, and are held as evidence failures when that evidence is absent.",
  },
  {
    path: "/admin/reliability/mail",
    methods: ["GET"],
    auth: "ADMIN_KEY",
    description: "Mail-leg receipt health. Inbound subscriptions and their canary retired on 2026-09-08; residual mail is receipt-only and old canary state cannot fail health. GET returns 503 for rejected emergency operations sends without retrying the dead mail rail. Routine reliability findings are Desk-only, while subscriber and product outbound delivery is unchanged. The Gmail forward leg remains unprobed.",
  },
  {
