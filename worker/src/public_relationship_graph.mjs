// Public, read-only civic relationship graph over linked source records.
...
export async function readPublicRelationshipGraph(db, canonicalEntityId, opts = {}) {
  ...
  const rows = (result?.results || []).map((row) => {
    ...
    if (Array.isArray(enriched.cross_spine_nodes)) crossSpineNodes.push(...enriched.cross_spine_nodes);
