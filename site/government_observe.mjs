...
export function buildObserveSurface(readModel = {}, scopeInput = {}) {
  ...
  return Object.freeze({ schema: "cityscroll.government_observe.v1", scope, observations: Object.freeze(observations), guides: OBSERVE_GUIDES });
}
...
export function renderObserveDocument(surface) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Observe City Government · CityScroll</title><meta name="description" content="Compare scheduled government observations with separate program guides."><link rel="canonical" href="https://cityscroll.org/observe/"><style>body{font:16px system-ui;max-width:72rem;margin:auto;padding:2rem;line-height:1.5}main{max-width:68rem}form{display:flex;gap:1rem;flex-wrap:wrap;padding:1rem;background:#f3f0e8}label{display:grid;gap:.25rem}.observe-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(18rem,1fr));gap:1rem}.observe-card{border:1px solid #bbb;padding:1rem}.observe-card dt{font-weight:700}.observe-card dl{display:grid;gap:.5rem}.observe-card dd{margin:0}.observe-guides li{margin:.75rem 0}.observe-guides span{display:block;color:#555}</style></head><body><main id="main"><p><a href="/">CityScroll</a></p><header><p>Observe</p><h1>Observe City Government</h1><p>Compare scheduled public observations, then choose the preparation step that fits.</p></header>${hiddenError}<form method="get" action="/observe/"><label>Body<select name="body"><option value="">All observation bodies</option>${OBSERVE_SOURCE_SYSTEMS.map((body) => `<option value="${body}"${scope.body === body ? " selected" : ""}>${BODY_LABELS[body]}</option>`).join("")}</select></label><label>Access<select name="access"><option value="">All access methods</option>${Object.entries(ACCESS_LABELS).map(([key, label]) => `<option value="${key}"${scope.access === key ? " selected" : ""}>${label}</option>`).join("")}</select></label><button type="submit">Apply filters</button></form><section aria-labelledby="scheduled-heading"><h2 id="scheduled-heading">Scheduled observations</h2>${rows ? `<div class="observe-grid">${rows}</div>` : "<p>No scheduled observations match these supported filters.</p>"}</section><section aria-labelledby="guides-heading"><h2 id="guides-heading">Program guides</h2><p>These guides explain access to programs that are not represented as upcoming dated sessions.</p><ul class="observe-guides">${guides}</ul></section></main></body></html>`;
}
