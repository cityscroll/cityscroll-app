...
function digItemHTML(kind, r, keywords, awarenessTools){
  if(kind==="exam"){
    ...
    return `<div class="digitem"><div class="dt"><a href="${examLink}">${escUiHtml(r.title||"")}</a></div>
      <div class="dm" lang="en" dir="ltr">${escUiHtml(meta)}</div>
      ${r.notice_url?`<div class="da" lang="en" dir="ltr">NOE posted</div>`:""}
      <div class="dc"><a href="${examLink}">${t("view_on_crol")}</a></div></div>`;
  }
...
function aLensFilter(){
  ...
  return {lens:"land", filter:{keywords:p?[p]:[], status:"all"}}; // rezonings — text match on the place
    ...
    return CrolScope.watchFromScope(CrolScope.scopeFromWatch(watch,{language:window.LANG||"en"}));
}
