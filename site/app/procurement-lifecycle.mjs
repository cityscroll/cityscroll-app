...
function lifecycleStageHTML(entry, timeline, notice, opts){
  ...
  if(publicStatus === "matched"){
    ...
    if((entry.stage === "award" || entry.stage === "pending") && d.vendor){
      extraHTML += `<div class="vend">${t("awarded_to")} <b lang="en" dir="ltr">${globalThis.v?.(d.vendor)||escUiHtml(d.vendor)}</b></div>`;
    }
    ...
    if(entry.detail && Array.isArray(entry.detail.candidates)){
      const cands = entry.detail.candidates.map(c => {
        ...
        return `<div>${lifecycleMoney(amt)}${dt ? " · " + fdate(dt) : ""}${c.vendor ? ` · ${globalThis.v?.(c.vendor)||escUiHtml(c.vendor)}` : ""}${c.contract_id ? ` · <code>${c.contract_id}</code>` : ""}</div>`;
      }).join("");
...
function lifecycleOcpAwardHTML(data){
  ...
  if(ocp.status === "ambiguous"){
    ...
    if(Array.isArray(ocp.candidates) && ocp.candidates.length){
      cands = `<div class="lc-candidates">${ocp.candidates.map(c =>
        `<div>${money(c.amount) || "—"}${c.date ? " · " + fdate(c.date) : ""}${c.vendor ? " · " + (globalThis.v?.(c.vendor)||escUiHtml(c.vendor)) : ""}${c.request_id ? ` · <code>${escUiHtml(c.request_id)}</code>` : ""}</div>`
      ).join("")}</div>`;
    }
  ...
  let body = t("lifecycle_ocp_matched_html",{
    source: srcLink,
    vendor: d.vendor ? `<b lang="en" dir="ltr">${globalThis.v?.(d.vendor)||escUiHtml(d.vendor)}</b>` : "—",
