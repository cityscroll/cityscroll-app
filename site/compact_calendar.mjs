...
function weekdayIndex(iso) {
  return ((isoToEpochDay(iso) + 4) % 7 + 7) % 7;
}
...
function monthGrid(month) {
  ...
  const gridFrom = addDays(first, -weekdayIndex(first));
  const gridTo = addDays(gridFrom, GRID_LENGTH - 1);
...
function timeKey(occurrence) {
  return occurrence.starts_at && !isDateOnly(occurrence.starts_at) ? occurrence.starts_at : "";
}
...
export function buildCompactMonthView(occurrences, options = {}) {
  ...
  for (let week = 0; week < GRID_WEEKS; week += 1) {
    ...
    for (let weekday = 0; weekday < WEEK_LENGTH; weekday += 1) {
      const date = addDays(gridFrom, week * WEEK_LENGTH + weekday);
...
function timeLabelFor(occurrence) {
  if (!occurrence.starts_at || isDateOnly(occurrence.starts_at)) return null;
