...
function normalizePlaceRoleFacet(values) {
  ...
  const { place_role: _drop, ...rest } = values;
  return rest;
}

/** Read the normalized place-role predicate, or null when none was requested. */
export function placeRoleFromScope(input) {
  return normalizeScope(input).facets.values.place_role || null;
}

export const NEAR_YOU_COMMON_LENSES = Object.freeze(["meetings", "land", "property", "rules", "money", "consultations"]);
...
function routableEntityRef(value) {
  ...
  if (/^notice:[A-Za-z0-9][A-Za-z0-9_-]{3,39}$/.test(ref)) return ref;
  if (/^pin:[A-Za-z0-9][A-Za-z0-9_-]{3,39}$/.test(ref)) return ref;
...
export function calendarFeedUnsupportedFilterFields(input = {}) {
  ...
  const unsupported = Object.keys(watch.filter).filter((key) => !allowed.has(key));
  if (watch.lens === "meetings" && Object.hasOwn(watch.filter, "availability")
      && !validateMeetingAvailability(watch.filter.availability).ok) {
    unsupported.push("availability");
  }
