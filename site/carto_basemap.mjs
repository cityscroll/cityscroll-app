...
export const CARTO_BASEMAP_API_KEY = "";
...
export function cartoBasemapTileUrl({ subdomain = "a", retina = false, apiKey } = {}) {
  const configuredKey = apiKey ?? CARTO_BASEMAP_API_KEY;
  const base = `https://${subdomain}.basemaps.cartocdn.com/rastertiles/light_all/{z}/{x}/{y}${retina ? "{r}" : ""}.png`;
  return hasConfiguredCartoBasemapKey(configuredKey) ? `${base}?key=${encodeURIComponent(configuredKey)}` : base;
}
