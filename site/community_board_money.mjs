...
export function renderCommunityBoardMoneyCard(card) {
  ...
  const provenance = sourceRefs.length
    ? `<details class="community-board-money-provenance"><summary>Sources and coverage</summary><div><ul>${sourceRefs.map((ref) => `<li>${sourceLink(ref, sourceLabel(ref))}${ref.fiscal_year ? ` · ${htmlEsc(cardFiscalYear(ref.fiscal_year))}` : ""}${ref.source_vintage?.payment_issue_date_through ? ` · through ${htmlEsc(cardDate(ref.source_vintage.payment_issue_date_through))}` : ""}</li>`).join("")}</ul><p>These are funds budgeted to and payments posted by this Community Board, not spending inside its Community District.</p></div></details>`
    : "";
  const identityNote = card.identity_unobserved && card.state !== "unmatched_identity"
    ? `<p class="community-board-money-note">Payment identity coverage is incomplete; this card does not infer a missing board payment total.</p>`
    : "";
  return `<section id="community-board-money" class="node-section node-card civic-object-section community-board-money-card" data-community-board-money="1" data-money-state="${htmlEsc(card.state)}" aria-labelledby="community-board-money-heading"><div class="community-board-money-heading"><div><p class="community-board-money-kicker">Board finances</p><h2 id="community-board-money-heading">Budget &amp; spending <span>${htmlEsc(headingDetail)}</span></h2></div></div><p class="community-board-money-boundary">Money budgeted to and paid by this Community Board — not spending inside its Community District.</p><p class="community-board-money-state">${htmlEsc(stateCopy(card))}</p><dl class="community-board-money-metrics">${card.budget?.adopted_amount != null ? metric("Adopted budget", cardCurrency(card.budget.adopted_amount), budgetDetail) : ""}${card.spending?.posted_amount != null && card.payment_through ? metric(spendingLabel, cardCurrency(card.spending.posted_amount), spendingDetailCopy) : ""}</dl>${card.spending?.posted_amount != null && !card.payment_through ? `<p class="community-board-money-unavailable">${htmlEsc(spendingLabel)}: unavailable</p>` : ""}${topPayees(card)}${identityNote}${provenance}</section>`;
}
