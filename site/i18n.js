...
const STRINGS = {
  en: {
    ...
    rule_stage_comment_closed: "Comment closed",
    ...
    rule_comment_btn: "Comment by {date}",
    ...
    rule_action_open_rule_page: "Open the official rule page",
    ...
    rule_event_official_source: "Official rule page",
    rule_source_nyc_rules: "NYC Rules",
    rule_event_provenance_html: "This timeline uses lifecycle dates published by {source}. Date-only deadlines are kept as New York calendar dates. CityScroll does not invent a closing time.",
    ...
    next_action_award_checkbook: "Open this contract on Checkbook",
    ...
    staffing_upcoming_deck: "These next application windows come from the precomputed DCAS exam guide. Open exams appear first.",
    staffing_explore_all_exams: "See all exam dates",
    staffing_notices_heading: "Live staffing notices, titles, and pay",
    staffing_notices_deck: "Search City Record personnel changes, recent payroll, and active civil-service titles. Live searches are labeled separately from the precomputed exam schedule.",
    ...
    staffing_exam_help: "New to civil-service exams? How it works",
    ...
    staffing_exam_redirect_html: "Civil-service exams follow a precomputed DCAS schedule — not the day-to-day City Record appointments above. Browse open and upcoming exams with deadlines, fees, and application links in the guide below.",
    ...
    career_loading: "Loading the precomputed DCAS exam guide…",
    ...
    career_noe_pending: "DCAS has scheduled this exam, but the detailed NOE is not available in this precomputed guide yet. Dates may change.",
    ...
    career_source_current: "Precomputed from official DCAS sources. Open exams checked {date}. Annual schedule current through {annual}.",
    ...
    entity_intel_method_note: "Each link names the source record that established it. Empty domains are honest gaps, not missing pages.",
    ...
    paper_trail_show_all: "Show all notices (chronological)",
    paper_trail_show_history: "Earlier stages",
    paper_trail_how_summary: "How this paper trail works",
    ...
    // Event-scoped official skim (#official/{id}?notice=&event=)
    ...
    about_staffing_formula_html: "We exact-match each exam number in the DCAS annual schedule to exam-level Civil Service List aggregates. For each match, the duration is <b>list established date − filing-period close date</b>. Within open-competitive and promotion cohorts (minimum 20 exams), the p10, median, and p90 are nearest-rank points on the empirical distribution. Smaller cohorts use the citywide distribution. A time-split backtest trains on lists established through 2024 and scores 2025–2026 resolutions. Per-exam dates appear only if at least 50 predictions resolve, p10–p90 coverage is 70%–90%, and any occurrence quintiles are monotone. Otherwise pages show only the cohort median. False-positive modes include schedule revisions, reused exam numbers, delayed lists, and active-list records with no establishment date. Applicant rows, names, scores, and ranks are never used.",
    ...
    std_p_timeline_lede: "Every time the published standard moves, or our own posture changes, we add a dated entry below instead of editing the numbers above. The record stays honest even when the target does not.",
