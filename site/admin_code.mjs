...
import { renderProvisionAuthorityCitations } from "./rulemaking_authority.mjs";
import { renderProvisionBackfillCoverage } from "./code_history_backfill.mjs";
...
export function renderAdminCodeProvisionDocument(row, { currentHref = null, changes = [], versions = [], as_of = null, mandateJoins = [], authority_citations = [], backfill = null } = {}) {
  if (!row) return null;
  ...
  const versionMarkup = scopedVersions.length
    ...
    : "";
  const citedMarkup = Array.isArray(authority_citations) && authority_citations.length
    ? `<section class="history" aria-labelledby="cited-as-authority"><h3 id="cited-as-authority">Cited as authority by</h3>${renderProvisionAuthorityCitations(authority_citations)}</section>`
    : "";
  ...
  const backfillMarkup = backfill?.provision_id === row.id ? renderProvisionBackfillCoverage(backfill) : "";
