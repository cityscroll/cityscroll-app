...
const cleanLandRegulatoryEffectValue = (value) => String(value ?? "").replace(/\s+/g, " ").trim();
...
export function normalizeLandRegulatoryEffect(value, fallback = "any") {
  const raw = cleanLandRegulatoryEffectValue(value).toLowerCase().replace(/[\s-]+/g, "_");
...
function districtIds(value) {
  const seen = new Set();
  const out = [];
  for (const match of cleanLandRegulatoryEffectValue(value).replace(SPECIAL_DISTRICT_WRAPPER, " ").matchAll(DISTRICT_TOKEN)) {
...
export function extractZoningDistrictPairs(projectBrief) {
  const brief = cleanLandRegulatoryEffectValue(projectBrief).replace(SPECIAL_DISTRICT_WRAPPER, " ");
  if (!brief) return [];
  const candidates = [];
...
function explicitOverlayPair(brief) {
  ...
  const match = cleanLandRegulatoryEffectValue(brief).match(new RegExp(
...
function stampedResult(record) {
  const effect = cleanLandRegulatoryEffectValue(record?.regulatory_effect).toLowerCase();
...
export function deriveLandRegulatoryEffect(record = {}) {
  ...
  const brief = cleanLandRegulatoryEffectValue(record.project_brief);
