...
The retained numeric sequence is the historical inventory key used by filenames and receipts, not a current priority. Reader-value and access-method columns preserve the August 2026 observation vocabulary; the control register is authoritative for objectives, sequencing, and status.
...
## Measured frontier inventory

Legacy `RC` labels are retained only as observation identifiers. They do not mean an item is ready, proposed, or owned.
