...
1 static site (`site/index.html` + window-sized `site/app/` modules + `site/data.html`) + 1 Cloudflare Worker, 7 lenses, public and operator API routes plus an inbound-email handler and queue consumer, 3 daily cron triggers (the 13:00 UTC chain is ingest → normalization/materialization → edge prewarm → queue fan-out), 4 KV namespaces + 1 D1 database (notices mirror + prior-cycle cache) + 2 Analytics Engine datasets + 2 queues, 6 secrets, 2 hard send caps — under one hard rule: no required accounts, no fingerprinting, no third-party trackers, and no visitor profiles beyond the first-party `cs_visitor` cookie's private, 30-day-bounded search-execution receipts; CityScroll-owned materialized read models serve ordinary views, while public-source access and durable snapshots provide graceful degradation and reproducibility.
