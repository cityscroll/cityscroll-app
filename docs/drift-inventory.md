...
| 12 | Suggestion-chip static fallback set (which live-validated example queries show per lens when the worker's daily `/suggestions` KV result is unreachable) | `site/app/search-share.mjs` `NL_SUGGESTIONS_FALLBACK` | `worker/src/lib/suggestions.mjs` `FALLBACK_INDICES` (+ `SUGGESTION_POOL`) | In-code floors are last-resort; live indices live in ALERT_STATE KV. `test/contract/suggestion_fallback.test.mjs` checks site/worker floor parity. |
