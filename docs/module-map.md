...
| `site/primary_document_view.mjs` | Shared document shell for build-rendered Now and Browse defaults. `site/browse_surface_contracts.mjs` is the identity registry for People + organizations, Staffing, and Exams; each canonical route has one direct builder and one controller, while legacy inputs are translated by intent at the ingress edge. `site/people_organizations_surface.mjs`, `site/staffing_surface.mjs`, and `site/exams_surface.mjs` independently own their row adapters, filters, and read-model builders; they depend on the presentation-only `site/browse_view.mjs` for byte-stable list/card HTML and may not import one another. `site/contextual_suggestions.mjs` renders data-derived next moves over the same scope; `tools/build_primary_documents.mjs` materializes them. |
| `site/search_document.mjs` | Standalone `/search/` enhancement for source-backed topic candidates and the six-family keyword fallback. `site/search_lens_handoff.mjs` carries raw and normalized topic scope, place/time context, typed entity constraints, and exact source evidence from admitted keyword SearchDocuments into established Browse routes without another state store. `worker/src/search.mjs` returns the family envelopes; `site/keyword_matcher.mjs` owns resolved literal terms, reviewed synonym expansions, and offset-backed evidence, and typed producers feed the compressed family shards in `worker/src/data/keyword_search_index_shards/`, refreshed by `node tools/build_keyword_search_index.mjs`. |
| `site/search_intent.mjs` | Read-only `cityscroll.search_intent.v1` projector over scope-v0, `resolveKeywordQuery`, and NL `sanitize`. It does not change those compilers or wire `/search` / `/nl`. |
| `site/ask_cited_synthesis.mjs` | Quote-only Ask card over `retrieveCitedPassages`. `/nl` emits `cited_quotes` as a sibling; the card quotes exact-join matched passages and never generates answers, relationships, or legal conclusions. |
| `site/semantic_topic_search.mjs` | Pure admission boundary for the versioned `/search/candidates` envelope. It validates sr1 corpus and sr2 passage receipts, rejects scores and unsafe evidence, and groups candidates only by declared source family; `site/search_document.mjs` owns the `/search/` rendering and legacy keyword fallback. |
| `site/derived_feature_rollup.mjs` | Pure incremental counts, date spans, lifecycle buckets, and freshness facts for materialized civic rows; keeps corpus totals separate from bounded previews. `site/civic_time_ledger.mjs` owns the narrow PASSPort RFx dependency registry and selective ledger/rollup rematerialization receipt over this same contract. |
...
| `site/assertion_inspector.mjs` | Resident-safe assertion/subject hydration over the versioned provenance graph; owns the `/assertions/` view contract, standable-edge gate, evidence traversal, and public redaction. Pages edge owns request routing and snapshot loading. |
...
| `site/universal_search_federator.mjs` | Pure cross-lens rank/merge plus the machine coverage receipt: observed/complete counts, per-lens and entity-type counts, snapshot boundaries, incomplete-lens states, and the requested-scope receipt over the closed registered-lens allowlist. `capabilities/federated_search.mjs` owns the versioned scope input. `site/universal_search_coverage_receipt.mjs` projects that receipt into resident copy consumed by `site/search_document.mjs`. |
| `site/app/staffing.mjs` | Staffing feed, roles, and personnel search. |
| `site/app/exams.mjs` | Civil-service exam guide/detail state, filters, cards, and exact `/exams/<id>/` selection. |
...
| `site/precomputed_address_geocoder.mjs` | Snapshot-only full-address parser and exact PAD range resolver. It loads one bounded `site/data/address-index/` shard, returns a publisher-backed BBL only when the result is unique, and leaves uncovered or ambiguous input `unknown`; `tools/build_geocoder_address_index.mjs` owns the official citywide PAD acquisition and daily materialization. |
...
| `site/action_path_v0.mjs` | Pure Action Path v0 projection over a validated registry action, grounded target, optional exact continuation, and provenance. No fetch, watch mutation, or actor fields. `site/council_hearing_matter_continuation.mjs` and `site/council_hearing_action_path.mjs` compose the Council proving slice; `site/civic_outcome_transition.mjs` emits source-backed rulemaking and matter transitions; `site/community_board_participation.mjs` keeps board-local Ways to participate source-qualified. Architecture receipt: `docs/civic-action-paths.md`. |
...
| `site/procurement_coverage_labels.mjs` | Record-varying procurement coverage copy: policy-matched small-purchase labels, M/WBE award-notice absence after a checked miss, and in-sentence observed-vs-publisher counts. Unknown methods and empty facets without a publisher figure stay silent. |
...
| `site/app/entities.mjs` | Official, agency, and vendor profiles; vendor phase timeline and forecasts. `site/vendor_footprint.mjs` renders the precomputed vendor footprint, separates confirmed links from name mentions, and composes count-receipted destination scopes; agency profiles use the pure `site/agency_connections.mjs` adapter to separate verified/tentative records and compose reload-safe scope links. Official profiles use `site/official_connections.mjs` for the fixed coverage gate, exact event→matter decision trail, and typed `votes_on` scope. |
| `site/agency_scope_route.mjs` | Pure adapter from typed agency entity facets to the source agency display value required by hydrated live lens controls. |
