...
| `site/primary_document_view.mjs` | Shared document shell for build-rendered Now and Browse defaults. `site/browse_view.mjs` owns bounded facet view models and HTML; `site/contextual_suggestions.mjs` renders data-derived next moves over the same scope; `tools/build_primary_documents.mjs` materializes them. |
