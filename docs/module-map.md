...
| `site/search_document.mjs` | Standalone `/search/` enhancement for the registered universal-search domain lanes. `worker/src/search.mjs` also returns six independent civic-family envelopes; `site/keyword_matcher.mjs` owns resolved literal terms and offset-backed evidence, and `site/search_lens_handoff.mjs` carries that topic, scope, and evidence into an existing typed lens without another store. Typed producers feed the compact `worker/src/data/keyword_search_index.json`, refreshed by `node tools/build_keyword_search_index.mjs`. |
