...
| `site/app/entities.mjs` | Official, agency, and vendor profiles; vendor phase timeline and forecasts. `site/vendor_footprint.mjs` renders the precomputed vendor footprint, separates confirmed links from name mentions, and composes count-receipted destination scopes; agency profiles use the pure `site/agency_connections.mjs` adapter to separate verified/tentative records and compose reload-safe scope links. |
