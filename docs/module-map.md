...
| `site/app/entities.mjs` | Official, agency, and vendor profiles; vendor phase timeline and forecasts. `site/vendor_footprint.mjs` is the vendor-route-lazy, strong-link-only footprint renderer and scope composer; agency profiles use the pure `site/agency_connections.mjs` adapter to separate verified/tentative records and compose reload-safe scope links. |
