...
| `site/primary_document_view.mjs` | Shared document shell for build-rendered Now and Browse defaults. `site/browse_surface_contracts.mjs` is the identity registry for People + organizations, Staffing, and Exams; its temporary compatibility projection keeps the current shared runtime stable during incremental separation. `site/people_organizations_surface.mjs` and `site/exams_surface.mjs` independently own their row adapters, filters, and read-model builders; both depend on the presentation-only `site/browse_view.mjs` for byte-stable list/card HTML and may not import each other. `site/contextual_suggestions.mjs` renders data-derived next moves over the same scope; `tools/build_primary_documents.mjs` materializes them. |
