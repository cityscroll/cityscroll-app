...
| `site/primary_document_view.mjs` | Shared document shell for build-rendered Now and Browse defaults. `site/browse_surface_contracts.mjs` is the identity registry for People + organizations, Staffing, and Exams; its temporary compatibility projection keeps the current shared runtime stable during incremental separation. `site/browse_view.mjs` owns bounded facet view models and HTML; `site/browse_reuse_surfaces.mjs` adapts Exams-alias and People-list typed civic objects into that same view; `site/contextual_suggestions.mjs` renders data-derived next moves over the same scope; `tools/build_primary_documents.mjs` materializes them. |
