...
This directory holds the source-owned inputs behind the repository governance
...
introduces `docs/repository-governance/cutover.v1.json` into version control
fails the check.
...
`node tools/governance_cutover_receipt.mjs --check` re-derives every input from the
