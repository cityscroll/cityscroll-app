...
| `data.html` | `fetch` to `data.cityofnewyork.us/resource/dg92-zbpx.json` | `live-only` | Live analytics strip intentionally reflects latest notice corpus for current session. | No migration in this batch. Candidate to move to worker materialized `/data-strip` endpoint; estimated effort 2 days. |
