...
| `index.html` Money default open RFPs | `fetch("data/money_default_open.json")` then hybrid SODA | `inline-at-build` + hybrid live | Default open solicitations 40; filters stay live. | **Migrated (wave 2 remaining):** snapshot first paint; client drops past-due rows; live refresh without re-autoSelect. |
| `index.html` Money agency dropdown | `fetch("data/money_procurement_agencies.json")` then hybrid SODA | `inline-at-build` + hybrid live | Procurement agency names are stable between builds. | **Migrated (wave 2 remaining):** snapshot first; live SODA group-by refreshes. |
| `index.html` Staffing default hires | `fetch("data/staffing_default_hires.json")` then hybrid SODA | `inline-at-build` + hybrid live | Last-80 APPOINTED notices for appointments feed. | **Migrated (wave 2 remaining):** snapshot first; keyword/payroll stay live. |
| `stats.html` | `GET /stats` edge cache | `edge-materialized` + cron prewarm | Product counters (not city Open Data); cold multi-KV fan-out. | **Prewarm (wave 2 remaining):** daily `prewarmStats` after digest so public counters stay warm. |
