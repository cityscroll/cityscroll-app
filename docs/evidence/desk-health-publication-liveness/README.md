...
The current live production observation is the newest dated envelope in this directory (`production-watchdog-read-YYYY-MM-DD.json`, with a `THHMM` suffix when the same UTC day already has an envelope). Refresh it with `CITYSCROLL_ADMIN_KEY_FILE=/path/to/key node tools/capture_desk_publication_production_read.mjs`, which records scheduled Deploy Cloudflare Pages runs, Reliability watchdog observer cycles, the scheduler heartbeat, the private Desk destination, and any retained per-run `cityscroll.desk_publication_receipt.v1` files from those Pages artifacts. The liveness reader counts `consecutive_unattended_publication_cycles`, not GitHub run summaries. Observer cycles do not count as publication; only a scheduled or dispatched Pages run may produce a publication heartbeat. [`production-watchdog-read-2026-09-10.json`](production-watchdog-read-2026-09-10.json) is the earlier 2026-09-10 read, kept as history. [`production-watchdog-read.json`](production-watchdog-read.json) is the 2026-09-06 historical read. Neither is current.
