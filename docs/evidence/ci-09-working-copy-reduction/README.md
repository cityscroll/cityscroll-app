...
A fresh reduced profile provisions in **7.1 s** and charges **202.2 MB** of
disk, against **35.2 s** and **1,346.1 MB** for the full-checkout control at the
same revision: **4.9x faster and 6.7x smaller**. Both dominants moved. Git
...
The saving is not bought by weakening anything: complete commit history
survives, the Worker unit family runs **1,755 tests with 1,754 passing and 1
skipped in both profiles** — identical counts, which is what rules out a pass by
omission — and the twelve gate classes exercised here all pass in whichever
profile they belong to. The four classes the profile cannot support are refused
before they run, not silently degraded.
...
| Field | Value |
| --- | --- |
| Revision | `b16d7e92235b2f2d959dad80d58aaf123fbbbbb3` |
...
Full detail, including the definitions of each byte measure, is in
...
| --- | ---: | ---: | ---: |
| Materialised by the profile | 3,765 | 141.28 | 18.3% |
| Deferred to explicit hydration | 310 | 230.05 | 29.9% |
| Left out of the profile entirely | 1,901 | 628.87 | 81.7% |
| **Tracked payload at this revision** | **5,666** | **770.15** | 100.0% |
| of which tracked `site/data` | 689 | 445.53 | 57.9% |
...
| --- | ---: | ---: | ---: |
| Git objects and repository metadata | 38.78 | 37.49 | 486.76 |
| Tracked `site/data` payload | 60.85 | 60.85 | 445.53 |
| Tracked `site/` payload outside `site/data` | 17.13 | 17.13 | 76.11 |
| Tracked payload elsewhere (tests, tools, docs, artifacts) | 62.21 | 62.21 | 212.10 |
...
| **Declared total, logical** | **495.42** | **494.12** | **1,572.39** |
| Declared total, allocated | 517.13 | 516.01 | 1,609.34 |
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| Reduced card-work profile | 3 | **202.2** | 200.0 | 1,022.9 | 197.5 | yes |
| Card-work profile, opt-in `--depth 1` | 3 | **201.0** | 198.6 | 1,022.8 | 196.4 | yes |
| Full-checkout control | 3 | **1,346.1** | -3,083.2 | 1,348.7 | 1,342.8 | no |
...
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| Reduced card-work profile | 3 | prepare | 773 | n/a | 768 | 793 |
| Reduced card-work profile | 3 | sparse | 254 | n/a | 241 | 256 |
| Reduced card-work profile | 3 | checkout | 3,562 | n/a | 3,373 | 3,633 |
| Reduced card-work profile | 3 | install | 2,540 | n/a | 2,493 | 2,660 |
| Reduced card-work profile | 3 | **total** | **7,102** | n/a | 7,042 | 7,202 |
| Card-work profile, opt-in `--depth 1` | 3 | prepare | 436 | n/a | 422 | 461 |
| Card-work profile, opt-in `--depth 1` | 3 | sparse | 1,152 | n/a | 1,085 | 1,194 |
| Card-work profile, opt-in `--depth 1` | 3 | checkout | 9,235 | n/a | 8,579 | 9,386 |
| Card-work profile, opt-in `--depth 1` | 3 | install | 2,568 | n/a | 2,525 | 2,726 |
| Card-work profile, opt-in `--depth 1` | 3 | **total** | **13,349** | n/a | 12,720 | 13,700 |
| Full-checkout control | 3 | prepare | 30,669 | n/a | 30,635 | 37,880 |
| Full-checkout control | 3 | checkout | 1,970 | n/a | 1,731 | 2,137 |
| Full-checkout control | 3 | install | 2,556 | n/a | 2,505 | 2,975 |
| Full-checkout control | 3 | **total** | **35,161** | n/a | 34,905 | 42,992 |
...
| `full-checkout-only-refused` | card-work | 3 | A gate class the profile does not support is refused before it runs, with the reason and the command that provisions the control. |
...
| --- | --- | --- |
| **Partial clone, `--filter=blob:none`** | Git metadata 486.8 to 38.8 MiB; object transfer 30.7 s to 0.8 s; complete history preserved (1,583 commits reachable in both profiles) | **Adopted as the default.** It is the only variant that takes the metadata cost down without taking history with it |
| **Shallow clone, `--depth 1`** | Metadata 37.5 MiB, barely below the partial clone's 38.8 MiB, and **total provisioning is nearly twice as slow**: 13.3 s against 7.1 s, because a shallow partial clone hydrates blobs in far smaller batches. It also cannot resolve an ancestor beyond the shallow boundary until unshallowed | **Kept as an opt-in, rejected as the default.** CI-08 ranked shallow first on its own numbers; combined with a sparse profile it loses to the partial clone on both axes |
| **Durable shared-object alternate** | Not adopted | **Rejected.** An alternate binds a checkout to another object store's lifetime. A partial clone reaches the same metadata reduction while depending only on the durable remote, and CI-08's own boundary forbids leaving a worktree dependent on a disposable object source |
| **Naive sparse cone, CI-08's `worker tools .github test`** | Working tree 79.5 MiB, but the Worker unit family fails outright | **Rejected, as CI-08 recorded.** It is the failure this card had to solve, not a profile to adopt |
...
| **Reducing the dependency view** | 315.2 MiB reported, about 2 MiB charged, unchanged across all three profiles | **Not a target,** as CI-08 established. CI-07 already solved this category on a filesystem class that can share |
...
**What stays full-checkout only.** Four gate classes were run against the
control here and are refused in the reduced profile, and five more are declared
full-checkout-only in `tools/card-profile/closure.v1.json` with their reasons:
the site standards family, the site and contract unit suites, the
generated-document and source-contract checks, the evidence-placement receipt
(measured at 2,571 tracked paths and 601.8 MiB, close to the whole tree), the
reading-level ratchet, the accessibility and browser families, the generated
Pages build, the release-surface and deployment checks, and any guard needing an
ancestor beyond a shallow boundary. **CI is unchanged and continues to use the
full checkout.**
...
the profile would charge roughly 495 MB rather than 202 MB — over the target.
The reduction this card implements is unaffected either way, because it is a
reduction of Git metadata and tracked payload; but the headline figure would not
survive that filesystem class, and saying so is part of the measurement.

**The free-space delta is noisy on a shared host.** One full-checkout trial
recorded a negative delta because another process freed more space than the
trial consumed. The median is the reported figure and the deterministic
allocated-minus-dependency-view cross-check agrees with it to within 2.4%, which
is why the claim does not rest on a single trial.

**The closure is derived from one recorded run per gate class.** A branch a
recorded run did not exercise, and that no scanned source names, is not in the
closure. That is a real limit, and it is the one the sentinel exists for: such a
path fails closed with the hydrate command rather than being skipped. Re-record
with `node tools/verify_card_profile.mjs --record <class> -- <command>` when a
gate class grows a new input.

**Hydration needs the network.** A deferred blob is served by the promisor
remote. An offline reduced checkout can run every supported gate class, because
their inputs are already materialised, but it cannot hydrate.
