...
| **Durable shared-object alternate** | Not adopted | **Rejected.** An alternate binds a checkout to another object store's lifetime. A partial clone reaches the same metadata reduction while depending only on the durable remote, and CI-08's boundary forbids leaving a worktree dependent on a disposable object source |
