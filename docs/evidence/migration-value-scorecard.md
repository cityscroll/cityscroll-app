...
| Persistent-failure detection window | **Derived from configuration** | The manual Pages fallback runs smoke and route parity with a **12-minute retry window** and 20-second retry interval; Cloudflare-native builds rely on the provider build receipt plus scheduled public-origin regression | `.github/workflows/deploy-cloudflare-pages.yml`, `docs/release/cloudflare-native-builds.md`, `tools/live_url_smoke.mjs`, and `tools/pages_route_parity.mjs` | An active HTTP probe can finish just after the configured window; no controlled failing deployment was introduced for this baseline |
