...
| Downstream assumptions | This contract already shipped (PR 1298). Generic envelope alias is `person:community-board:{board}:{key}` (`ontology/person.mjs:226-249`; `site/community_board_constellation.mjs:187-189`). Capability allowlist: `person.identity` + `community-board.roles` only (`ontology/person.mjs:47`). |
