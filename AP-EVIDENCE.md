...
This delivery extends AP-01 through AP-09 and AP-12: the versioned projection contract, Checkbook source-native dimension profile, population materialization, the Contracts Compare / Overview reader view, Registration timing, City Record coverage, vendor concentration, independent payments, and agency fiscal context. AP-10 remains a deferred future wave.

## AP-12 — Agency fiscal context

The agency profile now includes a static **Agency fiscal context** section. It joins the existing registered-contract and payment projections to AP-11's IBO materialization only through the shared canonical agency identifier. The committed adapter is `site/agency_fiscal_context.mjs`; its bounded materialization is `site/data/agency_fiscal_context.json`, generated by `node tools/build_agency_fiscal_context.mjs`.

The section keeps IBO actual expenditures, Personal Services, Other Than Personal Services, fiscal-year-end actual full-time positions, current/original registered contract value, and actual payments as separate measures. It shows source vintage, publisher units, fiscal-year convention, snapshot/population definitions, missing/non-overlapping years, independent agency ranks, and links to contributing Contracts/payment scopes. It does not publish an outsourcing or efficiency ratio, staffing-replacement claim, or causal interpretation.

- Exact-join coverage: 57 of 61 agency identities have IBO fiscal history; four recognized agency identities explicitly render `Fiscal context: Unknown`.
- Source coverage: IBO fiscal history is FY1980–FY2022 with FY2022 publisher vintage; the current registered-contract snapshot covers FY2025–FY2027 and the payment projection covers FY2026, so the current committed snapshots have no overlapping fiscal years. The table renders those non-overlaps as `Unknown`, never as zero.
- Independent rank proof: the context records separate latest available snapshots for IBO actual expenditures (FY2022), IBO staffing (FY2022), current registered contract value (AP-03 snapshot), and actual payments (FY2026).
- Before/after captures for Parks and Recreation: `artifacts/agency-fiscal-context/before-390.png`, `before-1440.png`, `after-390.png`, and `after-1440.png`; receipt: `artifacts/agency-fiscal-context/capture-receipt.json`.
